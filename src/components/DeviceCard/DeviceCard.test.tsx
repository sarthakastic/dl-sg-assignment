...
describe('DeviceCard', () => {
  ...
  it('allows entering the device type', () => {
    const deviceTypeInput = screen.getByLabelText(/Device type/i) as HTMLInputElement; 
    fireEvent.change(deviceTypeInput, { target: { value: 'Laptop' } });
    expect(deviceTypeInput.value).toBe('Laptop');
  });

  it('toggles BYOD option', () => {
    const toggleCheckbox = screen.getByLabelText(/Toggle if bringing your own device for Device 1/i);
  ...
  it('allows entering the serial number when BYOD is enabled', () => {
    ...
    const serialNumberInput = screen.getByLabelText(/Serial number/i) as HTMLInputElement; 
